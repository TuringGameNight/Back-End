...
RSpec.describe 'GET Game Night' do
  it 'gets the data about a game night' do
    ...
    expect(game_night_data[:data][:attributes][:confirmed_attendees].first[:name]).to eq(user2.name)
    expect(game_night_data[:data][:attributes][:confirmed_attendees].first[:email]).to eq(user2.email)
    expect(game_night_data[:data][:attributes][:confirmed_attendees].second[:id]).to eq(user1.id)
    expect(game_night_data[:data][:attributes][:confirmed_attendees].second[:name]).to eq(user1.name)
    expect(game_night_data[:data][:attributes][:confirmed_attendees].second[:email]).to eq(user1.email)
    ...
    expect(game_night_data[:data][:attributes][:pending_attendees].first[:id]).to eq(user3.id)
    expect(game_night_data[:data][:attributes][:pending_attendees].first[:name]).to eq(user3.name)
    expect(game_night_data[:data][:attributes][:pending_attendees].first[:email]).to eq(user3.email)
  end
